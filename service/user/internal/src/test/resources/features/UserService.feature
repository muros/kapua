...
Scenario: Retreive default lockout policy value 
    Default lockout policy is set to true and true should be returned as
    default value.

    When I retreive "boolean" metadata with id "lockoutPolicy.enabled" in scope 42 
    Then I receive "boolean" metadata with value "true" 

Scenario: Retreive default lockoutPolicy maxFailures value 
    Default lockout policy maximum failures is number of consecutive login failures before
    the user gets locked. Valid if lockout poilcy is enabled. Default value is 3.

    When I retreive "integer" metadata with id "lockoutPolicy.maxFailures" in scope 42 
    Then I receive "integer" metadata with value "3" 

Scenario: Retreive default lockoutPolicy resetAfter value 
    ...
    When I retreive "integer" metadata with id "lockoutPolicy.resetAfter" in scope 42 
    Then I receive "integer" metadata with value "3600" 

Scenario: Retreive default lockoutPolicy lockDuration value 
    Default lockout policy amount of time in seconds required after the last login failure
    to automatically unlock the user. Default vale is 10800.

    When I retreive "integer" metadata with id "lockoutPolicy.lockDuration" in scope 42 
